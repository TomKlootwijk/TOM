...
class CanonicalTests(unittest.TestCase):
    ...
    def test_definition_order_and_cycle_rejection(self):
        ...
        cyclic = [
            attach_hash({"id": "a", "dependencies": ["b"]}),
            attach_hash({"id": "b", "dependencies": ["a"]}),
        ]
        with self.assertRaisesRegex(ValueError, "cycle"):
            definition_order(cyclic)

    def test_definition_hash_is_required(self):
        with self.assertRaisesRegex(ValueError, "missing content_hash"):
            definition_order([{"id": "unhashed", "dependencies": []}])
...
class EndToEndTests(unittest.TestCase):
    def test_polar_loop_reference_result(self):
        ...
        self.assertEqual(state.phi, 2117)
        self.assertEqual(state.sheet, 1)
        self.assertEqual(state.lineage, 1625236203)
    ...
    def test_catalog_and_crosswalk_cover_project_sources(self):
        ...
        self.assertEqual(cross["count"], 322)
        sources = {row["source"].split(":", 1)[0] for row in cross["rows"]}
        self.assertTrue({"SRC-A", "SRC-B", "SRC-C", "SRC-D", "SRC-E", "SRC-F", "SRC-G", "SRC-H"}.issubset(sources))
    ...
    def test_c_backend_matches_python_when_available(self):
        ...
        if os.name == "nt" and exe.read_bytes()[:4] == b"\x7fELF":
            self.skipTest("the available C backend is a Linux executable")
        expected = json.loads((ROOT / "examples/polar_loop.expected.json").read_text())["state"]
        actual = json.loads(subprocess.check_output([str(exe), str(program_path)], text=True))

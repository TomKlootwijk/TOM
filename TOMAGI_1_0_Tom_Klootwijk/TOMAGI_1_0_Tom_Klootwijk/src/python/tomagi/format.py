...
from .core import Cell, Opcode, Program, State
...
def loads(data: bytes) -> Program:
    ...
    if any(values[9:15]):
        raise ValueError("reserved TOMAGI header words must be zero")
    ...
    for _ in range(count):
        cell = Cell.from_words(_CELL.unpack_from(data, offset))
        if cell.opcode > int(Opcode.HALT):
            raise ValueError(f"invalid cell opcode {cell.opcode}")
        cells.append(cell)
        offset += CELL_SIZE

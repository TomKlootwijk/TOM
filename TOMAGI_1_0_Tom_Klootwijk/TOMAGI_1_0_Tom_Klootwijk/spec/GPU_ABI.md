...
A conforming backend must produce the same 16 final words as the Python oracle for the same `.tmg` bytes, initial state, seed and number of transitions. The included package proves this equality for Python and C99 on the polar-loop and exact-19 programs, including focused signed-edge regressions. The recorded validation compiles GLSL with glslang, structurally checks WGSL, and reports OpenCL as not run because Clang was unavailable. No physical GPU dispatch is claimed; that requires a host API, a compatible device and its shader toolchain.

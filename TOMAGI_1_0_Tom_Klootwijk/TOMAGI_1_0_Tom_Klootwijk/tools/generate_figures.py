...
def core_pipeline():
    ...
    ax.plot([centers[3], centers[3]], [y, 0.29], color=RED, lw=1.4)
    ax.text(centers[3],0.245,"deterministic route / perturbation;\nnot error correction or safe mode",ha="center",va="top",fontsize=8.7,color=RED)
...
def trace_figure():
    ...
    trace=result["trace"]
    opcode_rows=json.loads((ROOT/"spec/opcode_table.json").read_text())["opcodes"]
    opcode_names={int(row["code"]): row["name"] for row in opcode_rows}
    ...
    names=[opcode_names[int(row["opcode"])] for row in trace]
    palette=[PURPLE,RED,BLUE,GREEN,GOLD,TEAL]
    colors=[palette[i % len(palette)] for i in range(len(trace))]

...
Unknown `State64` fields reject. Missing fields are zero. Signed and unsigned
interpretation is the frozen TOMAGI 1.0 interpretation.
...
is the graph length. Negative ticks and missing entries reject. User flags are
narrowed to `u32`; seeded-profile bit 0 is always set, and emitted-byte bit 1 is
set exactly when `emit_bytes` is true.
...
- `len` accepts list, record, or string. `concat` joins two lists; `append`
  returns a list with one evaluated terminal item.
...
- Numeric comparisons compare exact rationals. Equality also permits arbitrary
  formal values and otherwise uses structural JSON equality. Boolean operations
  require booleans and short-circuit in declared order. `if` evaluates only the
...
- empty emitted bytes, more cells or output bytes than budgeted, or a root that
  does not produce a program; and

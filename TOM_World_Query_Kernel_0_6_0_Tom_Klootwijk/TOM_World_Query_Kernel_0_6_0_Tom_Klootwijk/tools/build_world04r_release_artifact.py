...
DOC = ROOT / "TOM_WORLD_QUERY_KERNEL_0_4_REBUILT_RELEASE.md"
SOURCE = ROOT / "examples/world04r/world04r_release_artifact.literal.json"
PROGRAM = ROOT / "examples/world04r/world04r_release_artifact.tmg"
MATERIALIZED = ROOT / "validation/world04r/TOM_WORLD_QUERY_KERNEL_0_4_REBUILT_RELEASE.materialized.md"
PY_TRACE = ROOT / "validation/world04r/world04r_release_artifact.python.trace.json"
C_TRACE = ROOT / "validation/world04r/world04r_release_artifact.c.trace.json"
EMIT_RECORDS = ROOT / "validation/world04r/world04r_release_artifact.emit_records.json"
PROOF = ROOT / "validation/world04r/world04r_release_artifact.proof.json"
...
def definition(
    ...
    *,
    ...
    value: dict[str, Any] = {
        ...
        "limits": {},
        "provenance": {
            "source": "TOM World & Query Kernel 0.4.1 corrective release documentation",
            "corrected_v03_archive_sha256": "a7103ec92596fd54198e4a902f078712cf8eafcdf1e45320bbdc02dd53947278",
...
def source_record(data: bytes) -> dict[str, Any]:
    ...
    entry = f"cell:world04r-release:{0:0{width}d}"
    digest = sha(data)
    definitions = [
        ...
        definition("doc:bytes", "literal-corrective-release-document", "seed-record", "bytes", ["tom:tokens"],
                   "construct", 0, "literal", {
                       "result_type": "bytes",
        ...
        definition("doc:state", "initial-state", "seed-record", "state64", ["tom:tokens"],
                   "construct", 2, "state64.construct", {"fields": {"lineage": "0x57303452"}},
                   tokens=["lineage"]),
        ...
        definition("doc:emit", "byte-emission", "bytes", "cell_graph", ["doc:bytes"],
                   "transform", 1, "emit.graph", {
                       "id_prefix": "cell:world04r-release",
                       "key_base": {"rho": 360000, "theta": 0, "tick": 0, "phi": 0},
                       "key_field": "rho",
                       "chunk_bytes": 4,
                       "byte_order": "little",
                       "halt_last": True,
        ...
        definition("program:world04r-release", "artifact-program", "state-graph-guard", "program",
    ...
    return {
        ...
        "title": "TOM World & Query Kernel 0.4.1 corrective release document literal artifact",
        "seed_genome": {
        ...
        "root_definition": "program:world04r-release",
        "budgets": {
            "max_definitions": 16,
            "max_cells": count,
...
def main() -> None:
    ...
    if os.environ.get("TOM_WORLD04R_REFRESH_ARTIFACT_SOURCE") == "1":
    ...
    if result is None:
        raise RuntimeError("corrective release artifact did not compile under seeded profile")
    data, state, trace, records = materialize_file(PROGRAM, MATERIALIZED)
    if data != source_bytes:
        raise RuntimeError("materialized corrective release document differs from authored bytes")
    ...
    c_record = json.loads(subprocess.check_output([str(C_EXE), str(PROGRAM), "--trace-json"], cwd=ROOT).decode("utf-8"))
    C_TRACE.write_bytes(canonical_bytes(c_record) + b"\n")
    if c_record != py_record:
        raise RuntimeError("corrective release-document Python and C traces differ")
    c_data, c_emit = materialize_trace(load(PROGRAM), c_record["trace"])
    if c_data != data:
        raise RuntimeError("corrective release-document C and Python materialized bytes differ")

    sidecar = PROGRAM.with_suffix(PROGRAM.suffix + ".compile.json")
    proof = {
        "schema": "TOM-WORLD-QUERY-KERNEL-0.4.1-RELEASE-ARTIFACT-PROOF",
        "status": "pass",
        "corrected_v03_archive_sha256": "a7103ec92596fd54198e4a902f078712cf8eafcdf1e45320bbdc02dd53947278",

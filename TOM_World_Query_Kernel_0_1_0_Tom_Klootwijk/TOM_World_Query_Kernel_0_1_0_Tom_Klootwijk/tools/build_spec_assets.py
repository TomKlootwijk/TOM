...
SRC_ROOT=Path('/mnt/data')
CATALOG_PATH=Path('/mnt/data/_srczip/ugts_spatial_distill_v0.1.0/ugts_spatial_distill_v0.1.0/ugts_spatial/resources/knowledge_catalog.json')
...
{"phase":4,"name":"topology-and-hinges","operators":["TOM.KLEIN","TOM.MOBIUS","TOM.HINGE"]},
{"phase":5,"name":"relations","operators":["TOM.CONE","TOM.SWEEP","TOM.CIRCLE","TOM.SPHERE","TOM.OVERLAP"]},
{"phase":6,"name":"routing-and-grammar","operators":["TOM.RADIX","TOM.LSYS","TOM.BRANCH","TOM.TRANSITION"]},
{"phase":7,"name":"output-and-replay","operators":["TOM.PROJECT","TOM.EMIT","TOM.LINEAGE","TOM.TRACE"]}
],
"rule":"Functional notation composes right-to-left. Binary cell files execute one stored cell left-to-right per tick. The stored sequence is authoritative."
}
...
"type":"object",
"required":["tomagi_version","entry","cells"],
"properties":{
...
"definition":{"type":"object","required":["id","kind","domain","codomain","dependencies","parameters"],"properties":{"id":{"type":"string"},"kind":{"type":"string"},"domain":{},"codomain":{},"evaluation_phase":{"type":"integer"},"dependencies":{"type":"array","items":{"type":"string"}},"parameters":{},"provenance":{},"content_hash":{"type":"string","pattern":"^sha256:[0-9a-f]{64}$"}},"additionalProperties":True},
"cell":{"type":"object","required":["id","key","op","args","next"],"properties":{"id":{"type":"string"},"key":{"$ref":"#/$defs/key"},"definition_ref":{"type":"string"},"op":{"oneOf":[{"type":"string","enum":["NOP","SET","JIT1","KIN2","PHI","TIME","SDF0","CONE","SPHERE","KLEIN","RADIX","HINGE","LSYS","PROJECT","EMIT","HALT"]},{"type":"integer","minimum":0,"maximum":15}]},"flags":{"$ref":"#/$defs/int"},"args":{"type":"array","minItems":4,"maxItems":4,"items":{"$ref":"#/$defs/int"}},"next":{"type":"array","minItems":2,"maxItems":2,"items":{"type":"string"}},"payload":{"$ref":"#/$defs/int"},"aux":{"$ref":"#/$defs/int"}},"additionalProperties":False}
}}
(ROOT/'spec/tomagi.schema.json').write_text(json.dumps(schema,indent=2)+"\n")

# Source register.
source_files=[
("SRC-A","1bit parity / lower-case phi / log-polar LUT / cone / Klein / L-system dialogue",SRC_ROOT/'1bit-parity-bit-lower-case-phi-jitter-log-encoded-polar-LUT-analytic-sweeping-cone-T-a-side-view-of-the-pyramid-a-circle-a-sphere-the-apex-binary-.pdf',"Literal SDF0-in-LUT chain and precedence."),
("SRC-B","UGTS-KC 3.6.2 SCLP",SRC_ROOT/'UGTS_KC_3_6_2_SCLP_Tom_Klootwijk.pdf',"Cone, log-polar kinematics, jitter, phi, Klein wrap, L-system, 64-bit keys."),
("SRC-C","UGTS-KC 3.6 literal referential substrate",SRC_ROOT/'UGTS_KC_3_6_Tom_Klootwijk.pdf',"Definition records, hinges, operation order, active-bit and pulse examples."),
("SRC-D","21 Ben Burgers Strikes Back",SRC_ROOT/'21BenBurgersStrikesBackTelNetNiet.pdf',"Zero-based timeline, split-phi fold, overlap lens and connector interpretation."),
("SRC-E","19 / binary threshold / Dutch phonetic dialogue",SRC_ROOT/'waarom heeft 19 drie lettergrepen en in binaire waarom het kantelpunt tussen 1 en 2 cijfers geometrisch.pdf',"Active bits, pulse triangle, log-polar and Klein motifs."),
("SRC-F","Chronological Synthesis of the Spherical Substrate Line",SRC_ROOT/'Chronological Synthesis of the Spherical Substrate Line.pdf',"Query-first relation, transition and lineage synthesis."),
("SRC-G","UGTS spatial distill and GSP4 package",SRC_ROOT/'ugts_spatial_distill_v0.1.0 (2).zip',"211-mechanism catalog, CPU oracle, GPU ABI/shader sources and deterministic event authority.")]
reg=[]
for sid,title,path,role in source_files:
    reg.append({"id":sid,"title":title,"file":path.name,"sha256":sha(path),"role":role})
...
# Keep the supplied 211-entry source catalog as a traceable input, not as runtime code.
if CATALOG_PATH.exists():

...
static void apply_klein(TomagiState *s, uint32_t flags) {
    ...
    if (odd) {
        if (flags & TOMAGI_FLAG_KLEIN_SOURCE_HALF_TURN) {
            ...
            s->theta = i32_add((int32_t)(TOMAGI_THETA_STATES / 2u), -s->theta);
        }
        s->phi = i32_add(0, -s->phi);
        s->orientation ^= 1u;
...
static int32_t cone_residual(const TomagiState *s, const TomagiCell *c) {
    ...
    int32_t half = c->arg3 < 0 ? -c->arg3 : c->arg3;
    int32_t a = c->arg0 - rho;
    int32_t b = rho - c->arg1;
    int32_t radial = a > b ? a : b;
    int32_t delta = cyclic_delta_i32(theta, center, (int32_t)TOMAGI_THETA_STATES);
    if (delta < 0) delta = -delta;
    int32_t angular = delta - half;
    return radial > angular ? radial : angular;
}
...
int tomagi_step(const TomagiProgram *program, TomagiState *s) {
    ...
    switch (c->opcode) {
        ...
        case TOMAGI_OP_JIT1: {
            ...
            s->branch = bit;
            int32_t delta = bit ? c->arg0 : -c->arg0;
        ...
        case TOMAGI_OP_LSYS: {
            ...
            s->phi = norm_i32(s->phi + chirality * turn_sign * c->arg0,
                              (int32_t)TOMAGI_PHI_STATES);
            s->vrho /= divisor;
            s->vtheta /= divisor;
            s->vtick /= divisor;

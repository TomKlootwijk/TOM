...
fn addi(a:i32,b:i32)->i32 { return bitcast<i32>(bitcast<u32>(a)+bitcast<u32>(b)); }
...
fn main(@builtin(global_invocation_id) id:vec3<u32>) {
  ...
  if(op==1u){s=setField(s,flags&15u,a0,false);} else if(op==2u){let h=mix32(params.seed^k.x^rotl32(k.y,13u)^s.q.z^c.tail.w); s.topo.z=countOneBits(h)&1u; s=setField(s,flags&15u,select(-a0,a0,s.topo.z!=0u),true);}
  else if(op==3u){s.v+=c.args; s.q+=s.v;} else if(op==4u){let raw=addi(bitcast<i32>(s.q.w),a0); let w=floordiv(raw,i32(PHI_N)); s.q.w=u32(raw-w*i32(PHI_N)); if((u32(w)&1u)!=0u&&(flags&(1u<<4u))!=0u){s.topo.x^=1u;} s.outv.w=select(s.outv.w&~STATUS_PHI_WRAP,s.outv.w|STATUS_PHI_WRAP,w!=0); s.topo.z=select(u32(w)&1u,(s.q.w>>11u)&1u,(flags&(1u<<5u))!=0u);}
  else if(op==5u){let raw=addi(bitcast<i32>(s.q.z),a0); let w=floordiv(raw,i32(TIME_N)); s.q.z=u32(raw-w*i32(TIME_N)); s.topo.z=u32(w)&1u; if(w!=0){s.outv.x=mix32(s.outv.x^u32(w)^c.tail.w);}}
  else if(op==6u){s.outv.z=0u; s.outv.w|=STATUS_ZERO; s.topo.z=1u;} else if(op==7u){let rho=normi(bitcast<i32>(s.q.x),i32(RHO_N)); let th=normi(bitcast<i32>(s.q.y),i32(THETA_N)); let res=max(max(a0-rho,rho-a1),abs(cycdelta(th,normi(a2,i32(THETA_N)),i32(THETA_N)))-abs(a3)); s.outv.z=bitcast<u32>(res); s.topo.z=select(0u,1u,res<=0); s.outv.w=select(s.outv.w&~STATUS_CONE,s.outv.w|STATUS_CONE,res<=0);}
  else if(op==8u){let rho=normi(bitcast<i32>(s.q.x),i32(RHO_N)); let ph=normi(bitcast<i32>(s.q.w),i32(PHI_N)); var res=abs(rho-a0)-abs(a1); if(a3>=0){res=max(res,abs(cycdelta(ph,normi(a2,i32(PHI_N)),i32(PHI_N)))-abs(a3));} s.outv.z=bitcast<u32>(res); s.topo.z=select(0u,1u,res<=0); s.outv.w=select(s.outv.w&~STATUS_SPHERE,s.outv.w|STATUS_SPHERE,res<=0);}
  else if(op==9u){let rho=bitcast<i32>(s.q.x); let w=floordiv(rho,i32(RHO_N)); s.q.x=u32(rho-w*i32(RHO_N)); let odd=u32(w)&1u; if(odd!=0u){s.q.y=select(u32(i32(THETA_N/2u)-bitcast<i32>(s.q.y)),s.q.y+THETA_N/2u,(flags&1u)!=0u); s.q.w=bitcast<u32>(-bitcast<i32>(s.q.w)); s.topo.x^=1u; if((flags&2u)!=0u){s.topo.y^=1u;} s.outv.w|=STATUS_WRAP;}else{s.outv.w&=~STATUS_WRAP;} s.topo.z=odd; s=normalized(s);}
  else if(op==10u){let bi=u32(a0); if(bi<32u){s.topo.z=(k.y>>bi)&1u;}else{s.topo.z=(k.x>>(bi-32u))&1u;}} else if(op==11u){if((s.topo.z&1u)!=0u){s.q+=c.args; if((flags&1u)!=0u){s.topo.x^=1u;} if((flags&2u)!=0u){s.topo.y^=1u;} s=normalized(s);}}
  else if(op==12u){let sh=clamp(a1,0,30); let d=1<<u32(sh); let chir=select(1,-1,(s.topo.x&1u)!=0u); let sign=select(-1,1,(s.topo.z&1u)!=0u); s.q.w=u32(normi(bitcast<i32>(s.q.w)+chir*sign*a0,i32(PHI_N))); s.v=vec4<u32>(bitcast<u32>(bitcast<i32>(s.v.x)/d),bitcast<u32>(bitcast<i32>(s.v.y)/d),bitcast<u32>(bitcast<i32>(s.v.z)/d),bitcast<u32>(bitcast<i32>(s.v.w)/d));}
